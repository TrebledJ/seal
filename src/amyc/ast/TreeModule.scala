...
trait TreeModule { self =>
  ...
  // Variables
  // Encapsulates (possibly qualified) identifiers, local variables, function pointers, and constructors.
  ...
  case class Lambda(params: List[ParamDef], retType: Option[TypeTree], body: Expr) extends Expr
  ...
  enum Pattern extends Tree:
    ...
    case TuplePattern(args: List[Pattern]) // (arg1, arg2)
  export Pattern.*
  ...
  enum Type:
    ...
    case TupleType(types: List[TypeTree])
    case FunctionType(argTypes: List[TypeTree], retType: TypeTree)
    ...
    override def toString: String = this match
      ...
      case TupleType(types) => "(" ++ types.mkString(", ") ++ ")"
      case FunctionType(argTypes, retType) => 
        (argTypes.length match {
          case 0 => UnitType.toString
          case 1 =>
            argTypes.head.tpe match {
              case t@TupleType(_) => "(" ++ t.toString ++ ")" // Wrap an extra pair of parentheses.
              case t@FunctionType(_, _) => "(" ++ t.toString ++ ")"
              case t => t.toString
            }
          case _ => "(" + argTypes.mkString(", ") + ")"
        }) ++ " => " ++ retType.toString
