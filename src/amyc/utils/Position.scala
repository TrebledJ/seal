...
enum Position:
  case SourcePosition(file: File, line: Int, col: Int)
  case NoPosition

  def withoutFile: String = this match
    case SourcePosition(file, line, col) => s"$line:$col"
    case NoPosition => toString

  override def toString: String = this match
    case SourcePosition(file, line, col) => s"${file.getPath}:$line:$col"
    case NoPosition => "?:?"
